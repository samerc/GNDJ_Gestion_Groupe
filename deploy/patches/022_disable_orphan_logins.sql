...
-- log in but would see nothing). Disabling sets users.is_active=false + clears the refresh token; the member
...
UPDATE users u
SET is_active = false, refresh_token = NULL, refresh_token_expiry = NULL
FROM members m
