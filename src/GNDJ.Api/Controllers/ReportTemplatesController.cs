...
/// members.edit (chef d'unité / chef de groupe / super-admin).
/// </summary>
[Authorize]
...
public class ReportTemplatesController : BaseApiController
{
    ...
    /// <summary>Creates a report template. Requires members.edit (chef d'unité, chef de groupe, super-admin).</summary>
    ...
    /// <summary>Updates a report template. Requires members.edit (chef d'unité, chef de groupe, super-admin).</summary>
    [HttpPut("{id:guid}")]
    [HasPermission(Permissions.MembersEdit)]
    ...
    /// scopes and unit-leader access for a unit scope. Returns the file (PDF or Excel/CSV).</summary>
    [HttpPost("{id:guid}/generate")]
    [HasPermission(Permissions.MembersView)]
    public async Task<IActionResult> Generate(Guid id, [FromBody] GenerateReportBody body)
    {
        var result = await Mediator.Send(new GenerateReportFromTemplateQuery(id, body.ScoutYear ?? "", body.UnitId));
        if (!result.IsSuccess) return BadRequest(new { error = result.Error });
        return File(result.Value!.Data, result.Value.ContentType, result.Value.FileName);
    }

    /// <summary>Body for POST /report-templates/{id}/generate.</summary>
    public record GenerateReportBody(string? ScoutYear, Guid? UnitId);
}
