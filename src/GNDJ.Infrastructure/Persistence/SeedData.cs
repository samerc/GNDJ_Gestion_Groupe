...
public static class SeedData
{
    ...
    public static async Task SeedMissingSettingsAsync(GndjDbContext context)
    {
        ...
        var allSettings = new List<Setting>
        {
            ...
            new() { Key = "cotisation.full_amounts", Value = "{}", Category = "cotisations", Label = "Montant de la cotisation (par devise)", Description = "Montant de la cotisation dans chaque devise (ex. 30 USD et 2 500 000 LBP). Sert à distinguer « payé en entier » de « partiel » (chaque paiement compte comme une fraction du plein de sa devise) et pré-remplit le formulaire de paiement. Laisser vide pour ne pas suivre le plein (tout paiement compte comme payé).", ValueType = "json" },
            ...
            new() { Key = "cotisation.maitrise_amount", Value = "0", Category = "cotisations", Label = "Cotisation de la maîtrise", Description = "Montant de la cotisation payée par chaque membre de la maîtrise (chefs). Interne — non visible par les membres.", ValueType = "number" },
            // Global toggle: is the maîtrise expected to pay a cotisation this year? When OFF, maîtrise members no
            // longer see the cotisation on their Ma fiche, drop off the "à relancer" list, and aren't counted in the
            // association-dues report. (An individual maîtrise member can still be marked "ne paiera pas" separately.)
            new() { Key = "cotisation.maitrise_pays", Value = "true", Category = "cotisations", Label = "La maîtrise paie une cotisation", Description = "Si activé, les membres de la maîtrise (chefs) doivent payer la cotisation de la maîtrise. Désactivez pour que la maîtrise ne paie pas cette année : la cotisation disparaît alors de leur « Ma fiche » et ils ne sont pas relancés.", ValueType = "boolean" },
            ...
            new() { Key = "camp.game_locations", Value = "[]", Category = "camp", Label = "Lieux des jeux (Camp BP)", Description = "Les lieux où se déroulent les jeux (souvent les mêmes chaque année). Chaque jeu reçoit un lieu principal et un lieu de repli en cas de mauvais temps, choisis dans cette liste.", ValueType = "json_array" },
            new() { Key = "camp.etapistes_aines", Value = "false", Category = "camp", Label = "Routiers, caravelles et JEM comme étapistes", Description = "Si activé, les routiers, caravelles et JEM peuvent aussi être choisis comme étapistes d'un jeu (sinon : la maîtrise seulement).", ValueType = "boolean" },
            ...
            // New-year cleanup ("Nettoyage de nouvelle année", Paramètres → Passage): document types KEPT when the year
            // is reset (codes; the rest is exported then deleted) + whether the kept ones stay approved.
            new() { Key = "newyear.keep_document_types", Value = "[\"CI\"]", Category = "passage", Label = "Documents conservés à la nouvelle année", Description = "Types de documents (codes) conservés lors du nettoyage de nouvelle année ; les autres sont exportés puis supprimés.", ValueType = "json_array" },
            new() { Key = "newyear.keep_id_approval", Value = "true", Category = "passage", Label = "Garder la validation des documents conservés", Description = "Si désactivé, les documents conservés (carte d'identité) repassent « En attente » et doivent être revérifiés.", ValueType = "boolean" },
            ...
            new() { Key = "passage.first_meeting_date", Value = "", Category = "passage", Label = "Première réunion de l'année", Description = "Date de la 1ère réunion de l'année scoute (où se règle la cotisation). Utilisée dans l'email de réinscription envoyé aux membres. À définir chaque année.", ValueType = "date" },
            ...
            // Scheduled announcement banners shown at the top of the two login screens — a JSON array of messages,
            // each with its OWN optional start/end window ([{ "text": "…", "start": "yyyy-MM-dd"|null, "end": …|null }]),
            // so the CG can queue several banners at once. Independent (member vs applicant portal). Edited via a
            // dedicated editor in Paramètres → Connexion (category "login" is CG-editable). Empty [] = no banner.
            new() { Key = "login.member_messages", Value = "[]", Category = "login", Label = "Messages — connexion membres", Description = "Messages affichés en haut de l'écran de connexion des membres et chefs (« Espace membres »), chacun avec sa propre période d'affichage (dates de début/fin optionnelles).", ValueType = "json" },
            new() { Key = "login.applicant_messages", Value = "[]", Category = "login", Label = "Messages — connexion inscriptions", Description = "Messages affichés en haut de l'écran de connexion du portail des inscriptions (parents / « Demande d'inscription »), chacun avec sa propre période d'affichage.", ValueType = "json" },
            ...
            new() { Key = "notifications.retention_days", Value = "90", Category = "maintenance", Label = "Conservation des notifications lues (jours)", Description = "Les notifications déjà LUES plus anciennes que ce délai sont supprimées automatiquement chaque jour (0 = ne jamais supprimer). Les notifications non lues sont conservées. Évite que la table des notifications grossisse indéfiniment.", ValueType = "number" },
            new() { Key = "outbox.retention_days", Value = "30", Category = "maintenance", Label = "Conservation des emails/notifications envoyés (jours)", Description = "Les entrées de la file d'envoi (emails et notifications push) déjà ENVOYÉES ou en ÉCHEC plus anciennes que ce délai sont supprimées automatiquement chaque jour (0 = ne jamais supprimer). Les envois en attente sont conservés.", ValueType = "number" },
