      ...
      <Tabs value={activeTab} onValueChange={(v) => { setActiveTab(v); if (v !== 'profile' && v !== 'medical') setEditing(false) }}>
        ...
        <TabsContent value="contact" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>{member.addresses.length === 0 ? <p className="text-sm text-muted-foreground">Aucune</p> : <div className="space-y-2">{member.addresses.map(a => (<div key={a.id} className="flex items-center gap-3 rounded-md border p-3"><div className="flex-1"><span className="font-medium">{a.city}, {a.country}</span>{a.details && <p className="text-sm text-muted-foreground">{a.details}</p>}</div><div className="flex items-center gap-1">{a.isPrimary && <Badge>Principal</Badge>}<Tip content="Modifier"><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => openEditAddress(a)}><Pencil className="h-3.5 w-3.5" /></Button></Tip><Tip content="Supprimer"><Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setDeletingContact({ type: 'address', id: a.id, label: `${a.city}` })}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button></Tip></div></div>))}</div>}</CardContent>
          </Card>
        </TabsContent>
