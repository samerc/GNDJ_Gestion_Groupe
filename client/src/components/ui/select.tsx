...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      // Focus/hover language unified with Input (soft ring + border-ring, no offset) so text fields and
      // selects read identically side by side.
      "flex h-9 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm shadow-2xs ring-offset-background transition-colors data-[placeholder]:text-muted-foreground hover:border-primary/50 focus:outline-none focus:border-ring focus:ring-2 focus:ring-ring/40 focus:ring-offset-0 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
